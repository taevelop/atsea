...
`npm run build`는 웹 배포용 `dist/`와 단일 배포용 `dist-single/atsea3d.html`을 최신 소스로 함께 빌드합니다. 정적 호스팅에는 `dist/`를 사용합니다. 원본 Blender 파일, 제작 도구, 테스트 코드는 배포물에 들어가지 않습니다.
...
단일 HTML만 다시 생성할 때는 다음 명령을 사용합니다.
...
단일 파일 빌드는 JavaScript·Three.js·CSS·아이콘을 HTML에 넣고, GLB 모델 27개는 gzip으로 무손실 압축한 뒤 Base64로 내장합니다. 브라우저의 `DecompressionStream`으로 모델을 복원해 `GLTFLoader.parseAsync`에 전달합니다. Google Fonts 요청과 설치용 웹앱 매니페스트는 제외하며, 기존 시스템 글꼴 대체 설정을 사용합니다. 일반 웹 배포는 `npm run build`의 `dist/`를 사용합니다.

크기는 ZIP 전의 파일 크기 기준으로 빌드할 때마다 콘솔에 표시됩니다.
...
단일 HTML과 오프라인 테스트 결과 폴더는 개발 서버의 파일 감시 대상에서 제외됩니다. 오프라인 검증은 개발 서버 없이 실행할 수 있습니다.
...
일시정지 중에도 수심은 옮길 수 있습니다. 도감·희귀종·칭호·트로피와 조건을 달성한 뒤의 특별 먹이·메갈로돈을 그대로 지원합니다. Fish 슬라이더는 일반 물고기를 기준으로 상어·고래·돌고래를 제외한 수영 생물을 원래 비율에 맞춰 조절합니다. Sharks와 Coral 사이의 Whale 슬라이더는 고래와 돌고래의 합계 개체수를 조절하며, 최댓값은 Sharks와 같습니다. 상어와 바닥 장식도 각각 별도 슬라이더를 사용합니다.
