...
- **2D·3D 화면 전환:** 원본 ASCII 도안과 애니메이션 3D 모델로 같은 바다를 둘러봅니다. 3D를 준비하는 동안에도 2D로 플레이할 수 있습니다.
- **다양한 바다 생물:** 물고기·상어·거북·게·새우·돌고래·고래·심해 산갈치 등을 관찰합니다. 개체별 색상과 희귀종의 발광, 초롱아귀의 해저 조명, 포식 효과를 표현합니다.
- **낚시와 수집:** 생물의 목격·포획 기록을 도감에 모으고 희귀종·칭호·트로피를 수집합니다. 진행 조건을 달성하면 특별 먹이와 메갈로돈을 만날 수 있습니다.
- **바다 탐색과 조절:** 수심을 이동하고 생물 수·바닥 장식·속도·물빛을 조절합니다. 키보드·마우스·모바일 터치를 지원하며 배경화면 모드로 감상할 수 있습니다.
- **웹과 오프라인 실행:** 정적 웹사이트로 배포하거나 코드와 모델을 내장한 단일 HTML을 인터넷·웹서버 없이 실행합니다. 한국어와 영어를 지원합니다.
...
버전별 변경 사항은 [Version History](<Version History.md>)에서 확인할 수 있습니다.
